...
- **`docs/` 与 `deploy/` 另有一条**（`pnpm docs:backup`，`deploy-ecs.sh` 结尾自动跑）：
  ...
  排除 `docs/APIs`（厂商 PDF，可重新下载）与 `*.mp4` / `*.mov`（渲染产物，源工程还在）
  后整包约 0.5 MB。⚠️ 排除项的判据只有一条：**丢了是不是就再也没有了**。
  2026-09-11 一个 14 MB 的教程 mp4 进了 `docs/`，整包涨到 14.42 MB ——
  留 30 份就是 432 MB，与设计意图差两个量级。**被排除的视频因此没有异地备份**，
  要备份就单独传一次，别塞回 `docs/`。
...
- 视频额度是 **480p 等效秒**，不是裸秒数：`PLATFORM_DAILY_VIDEO_SECONDS`（默认 120）。
  ⚠️ 按裸秒算的话额度会**名不副实** —— 成本随分辨率放大得厉害（720p ×2.25、
  1080p ×5.06、4K ×20.25），同一份「120 秒」选 4K 就能烧掉 20 倍的钱，
  于是它从「每天最多花多少钱」退化成「最多生成多长的片子」，而后者不是我们要限的东西。
  所以 `unitsOf` 一律乘 `resolutionMultiplier()`，**每个调用点都必须把 `resolution` 传进来**
  （漏传按 480p 计 = 高分辨率被少算）
...
**owner 那半边也一样错，只是错法不同**：他是唯一能改平台列表的人，却在 A 电脑
删掉一个 provider 之后，B 电脑上那份旧列表原封不动，还会被防抖写回去**把 A 上的
删除盖掉**。所以 owner / 自部署走**版本号比较**：`GET /api/user-prefs/model-store`
返回信封 `{ userId, updatedAt, payload }`，服务端那份更新就采用它；本地反而更新
（上次写回没成功、或改完立刻关了页面）就主动补一次写回 —— 不补的话它永远出不去，
因为写回只由「状态变化」触发，而那份已经不会再变了。

⚠️ **`updatedAt` 必须是服务端时钟、且必须是落库后回读的值。** 两处都踩过同类坑：
浏览器本地时间可能与服务器差几分钟，两边各记各的就会「更旧的被判成更新」；
而 Drizzle 的 `mode:"timestamp"` 只存到**秒**，直接返回 `Date.now()` 会比读回来的大
最多 999 毫秒，于是客户端永远判成「本地更新」、**服务端的改动再也进不来**
（同约定 8i / 8j 的秒/毫秒）。`user-client-prefs.test.ts` 钉死。

**换账号**：zustand persist 的 key 是固定的 `"model-store"`、**不带 userId**，
同一台浏览器换账号时上一个人的列表会留在本地。客户端把「我手里这份是谁的、
对应服务端哪一版」记在独立的 `model-store-sync` 里（**不放进 persist** ——
放进去会被 `partialize` 带着写回服务端、被 `migrate` 的版本机制管着、
还会在更新时触发一轮写回），与服务端返回的 `userId` 一比不同就整体丢弃本地那份。
所以那个接口**没有数据时也要返回 userId** —— 「新账号还没配过」正是最该丢掉本地的时刻。

**托管模式下非管理员只写回默认模型**（`providers: []`）：列表是管理员那份的副本，
写回去只会在每个人的行里存一份 11 KB 的重复 JSON，排查时还容易被误认为是用户自己配的。

**空列表必须在界面上说清楚**：四个只有「--」的下拉框分不清是没配、没拉到、还是权限不够，
而平台模式（去找管理员）与自部署（自己往下配）的引导完全相反。
`DefaultModelPicker` 在一个可选模型都没有时按 `managed` 给出不同文案，不再渲染空下拉框。
...
| 2026-09-09 用 `git filter-repo` 移除了 `deploy/` 与 `output/` 的全部历史，**再次实测确认上一条** | 改写本身是干净的：213 提交一条不少，`main` 的**树哈希改写前后完全相同**（`187b7859…`），即只换提交 SHA、代码一字节未变；从 GitHub 重新 clone 全历史查得 `deploy/`/`output/` 提交 0 条、对象 0 个。**但旧 SHA `0fd27e5` / `3e1ea23` 仍返回 HTTP 200** | 流程固定为：① `git clone --mirror` 做完整备份（改写后它是唯一的旧历史副本，别急着删）；② 在**独立镜像**里跑 filter-repo，不直接动工作仓库；③ 先删掉不该推的 ref（本地工作树分支、`refs/codex/turn-diffs/*` 这类工具残留），否则它们会把旧对象拖回来；④ **必须比对树哈希**证明内容未变；⑤ 强推后在自己控制的两处（工作仓库 `.git` 与自建 git server）跑 `reflog expire --expire=now --all && gc --prune=now` 才是真删除（本次 git server 98M→39M、残留对象 0）—— **只有自己的仓库删得掉，GitHub 那边只能开工单** |
...
| 误判「这个 hydration 报错是 dev-only、线上没有」| 我拿生产环境的 `/zh/login` 和 `/zh/settings/prompts` 测出「零报错」就下了结论 —— 但**那两页根本不渲染出问题的组件**：生产是 `REQUIRE_AUTH=1`，未登录首页显示的是登录引导，`CreateProjectDialog` 压根没渲染。**「换个页面没复现」只有在那个页面确实包含问题组件时才算证据** | 排查顺序仍然是「先换未改动页面、再上生产构建复现」，但每一步都要先确认**被测页面真的包含那个组件**，否则测的是空气。另一类 hydration 问题（render 里读 `localStorage`/`window`）见上一条 |
| 页面级登录闸该放哪：**segment layout，不是 middleware** | middleware 跑在 **edge runtime，没有 node 的 `crypto`** —— 想在那里验 auth cookie 的签名，就得用 Web Crypto 再写一份 HMAC，于是仓库里有了**两份签名实现**，早晚漂移。而 segment layout 是服务端组件（node runtime），可以直接复用 `lib/auth.ts` 的 `parseCookieValue` | 统一走 `lib/auth-page-guard.ts` 的 `requirePageAuth(locale, next?)`。⚠️ 两条必须记住：① **未开 `REQUIRE_AUTH` 时不能拦** —— 自部署单机是匿名可用的，拦住等于废掉整个自部署场景；② **这是 UX 跳转，不是安全边界** —— 真正的准入在 API 那层（约定 8b），绕过跳转直接打接口一样会被挡 |
| 要加闸的 layout 是**客户端组件**时怎么办 | `project/[id]/layout.tsx` 原本是 `"use client"`，读不到 cookie，所以未登录访问项目链接只能得到 404 —— 不泄漏数据，但收藏了链接的人会撞进死胡同 | **拆成「服务端 layout（闸）+ 客户端外壳（UI，内容不动）」**，而不是把守卫下推到每个 page。关键收益：**服务端 layout 的 `params` 里有动态段**（这里是 `[id]`），`next=` 能指回**这个项目**而不是退化成首页。⚠️ 但要指到一个**真实存在的页面**：我第一版指的是 `/project/{id}`，而那一层根本没有 `page.tsx`（只有子路由），用户登录完直接落到 404 —— 比原来的死胡同还糟。改指默认入口 `/episodes`。⚠️ **不要图省事在上一层加 `project/layout.tsx`** —— 那一层拿不到 `[id]`，`next=` 只能退化成首页，等于把「登录后回到原处」这个唯一收益丢掉了。**子页也要精确回去时用路由组**：守卫放在父层就只能给一个笼统的 `next`（父层先执行，一 redirect 子层就没机会了）。把守卫下放到**每个叶子各自的 layout**，`/settings` 用 `(root)` 路由组拿到独享 layout（路由组不占 URL 段），`/settings/prompts` 用自己的 layout —— 两边就都能带精确的 `next`，且**不用动页面组件**。|
...
| 反例：这次机器**是真的停了** —— 怎么和上一条区分 | 2026-09-05 22:52 起机器完全不响应，症状与上一条一模一样（TCP 通、无 banner）。但这次是真故障：**`journalctl --list-boots` 出现 1 小时 50 分钟的断档**（boot -1 停在 22:52:03，boot 0 从 00:42:19 开始），而上一条那次是**单次启动连跑 28 小时、没有任何断档**。佐证：云助手 agent 心跳停在 22:51:02Z；VNC 全黑且敲键无响应；在**阿里云网络内**（Cloud Shell，带对照端口）测同样拿不到 banner —— 不是本地 VPN 在骗人 | **判据就是「日志有没有断档」**：`journalctl --list-boots` 一眼看出机器到底停没停，这比任何连通性测试都可靠，因为它是机器自己写的。连不上时的固定顺序：① `--list-boots` 看断档 → ② 云侧带对照组探测 → ③ 才动手。⚠️ **病因至今不明,不要编**：boot -3/-2/-1/0 **四次启动 OOM 全是 0**、无 panic / hung task / I/O error / EXT4 错误、**无任何 `No space left`**；日志停在我自己一次正常的 SSH 登入登出后。已知的只有两点：冻住时正在跑 `docker compose build`（buildkit session healthcheck 连续 fatal），以及内核在之后 1h50m 里一直能完成 TCP 三次握手。处置：先打快照再强制重启（10 秒恢复），并清掉 14.47 GB 构建缓存（24G→11G）把「空间不足」这个候选排除掉 |
...
| `deploy/` 被误提交进公开仓库（含服务器 IP） | **`echo "..." >> .gitignore` 追加到一个没有结尾换行的文件**，把最后一行 `deploy/` 和新增的 `.pnpm-store/` 拼成了 `deploy/.pnpm-store/` —— 不是多了一条错规则，而是**静默删掉了一条正确的规则**，同一次 `git add -A` 就把整个 deploy/ 提交了上去。`git status` 只显示「新增了几个文件」，看不出忽略规则被破坏 | `.gitignore` 还原为两条独立规则；`git rm --cached -r deploy/`。**凡是往逐行生效的文件（.gitignore / .env / authorized_keys）追加内容，必须先确认结尾换行**（`[ -s f ] && [ "$(tail -c1 f)" ] && echo >> f`）。另一个教训：`git ls-files | grep '^deploy/'` 这种检查要在**每次 push 之后**跑，而不是只在改 .gitignore 时跑 —— 这次正是靠部署后重新 clone 一份才发现的 |
| 画质增强实际产出 1080p，却被全程当成 720p | 路由调 `enhanceVideo()` 时**没传 resolution**，于是走 provider 的默认值（`volcengine-enhance.ts` 里是 `?? "1080p"`）。而 DB 的 `videoResolution`、界面按钮文案、历史标签、额度折算全都写死 720p —— 用户以为拿到 720p 实际拿到 1080p，额度只按 2.25 倍扣而真实成本是 5.06 倍。「已经是 720p 就拒绝」那条守卫还顺带让 720p 的片子再也升不到 1080p，而 provider 本来就支持 | 目标分辨率改为**显式参数**（body 的 `resolution`，默认 1080p = 与改前的实际行为一致），落库/额度/文案三处都跟着同一个 `target` 走；守卫改为 `resolutionRank(当前) >= resolutionRank(目标)` 才拒绝，于是可以逐级升。**教训：provider 有默认值而调用方不传，等于把行为交给了另一个文件的一行 `??`，而调用方这边的常量还在自说自话** |
...
| 整套设计 token **全站从未生效过**，而且完全无声 —— tsc / 构建 / 单测全绿 | Tailwind v4 取消了 `[--x]` 的隐式 `var()` 包装：`[--x]` 被当成无效的字面量色值，**一条 CSS 规则都不生成**（v3 里它等价于 `var(--x)`）。于是 981 处 `text-[--text-muted]` / `bg-[--surface]` / `bg-[--primary]` 全是空操作，muted 文字渲染成纯黑、surface 与 border 全透明，界面靠浏览器默认值兜底。**它看起来像是有设计的**，因为边框、圆角、阴影和 `bg-primary` 这类 `@theme` 主题工具类一直是好的 —— 所以没人发现。实测对照：`bg-[--primary]` 透明 / `bg-(--primary)` ✓ / `bg-primary` ✓ / `bg-[var(--primary)]` ✓ | 68 个文件 981 处统一改为 `(--x)`（改动完全对称，766 增 766 删，只换语法）。顺带修掉两个**从未定义过**的 token —— 只改语法它们仍然解析不到：`--bg`→`--background`（分镜页 sticky 头部，透明会让滚动内容透出来）、`--surface-alt`→`--surface`。⚠️ **判据：改了颜色没反应，先 `getComputedStyle` 看规则到底生成了没有，别靠肉眼。** 肉眼在这里是失效的 —— `--surface` 是 #F5F5F2，和白色几乎分不出来；验证生产构建则直接抓 `/_next/static/chunks/*.css` 数 `var(--x)` 的出现次数 |
| 部署时 `docker compose build` 把整机拖死 4 小时，而一切迹象都指向「我的网络坏了」 | 部署脚本在**旧容器还跑着**的时候构建（约占 640MB 真实内存），叠加服务器 `.env` 的 `NODE_BUILD_MEMORY=2048`，在 4 GB 机器上必然打爆 → 持续 swap 颠簸、`kcompactd0` 阻塞 >120 秒。内核活着（还在往控制台打日志、还能完成 TCP 三次握手），**用户态全饿死**：sshd 接受连接但不回 banner、3007 超时、云助手 agent 起不来、VNC 上 getty 连回显都做不到。⚠️ 脚本里那道内存预检是**摆设** —— `BUDGET = 可用内存 + swap`，而这台机器有 4 GB swap，于是 BUDGET 恒 ≥4000、**永远不触发**，可「把内存压进 swap」恰恰就是失败机制本身 | 构建前先 `docker compose stop`（腾出 640MB），且必须**停在内存测量之前**，否则量到的不是构建真正能用的；另加一道**不含 swap** 的真实内存底线（<1800MB 直接拒绝）。实测修复后：可用内存最低 1147MB、swap 用量全程 0、`pnpm build` 88 秒完成（此前是卡死 4 小时）|
| 排查上一条时我先后误判成「VPN 伪造连接」和「服务器冻结」，两次都错 | ① 开头的控制端口对照测试确实显示 VPN 在伪造 TCP 成功（未放行的 12345 也「连上了」），于是我把 banner 超时归给 VPN —— 但**伪造连接与服务器不回 banner 的症状一模一样**，这个判据在此处根本无法区分；② 我又拿「VNC 有登录提示符」断定系统活着 —— **静态登录符可能是卡死前留在帧缓冲里的画面，什么都不证明**；③ 把我拉回正轨的是用户一句「这个 VPN 一直能连这个 IP」，随后查安全组发现 22/3007 对 0.0.0.0/0 全放行，网络这条线本来就是干净的 | **判据要用能自证的**：㈠ 在 VNC 里敲一个字符看回不回显 —— 不需要任何凭据，直接区分「内核活/用户态死」；㈡ `journalctl --list-boots` **不能只看有没有新 boot** —— 这次没有新 boot，而是当前 boot 的日志停在 18:30:42、之后 4 小时空白，**「结束时间不是现在」才是卡死的铁证**；㈢ 云控制台 CPU 曲线（持续 97% 却毫无网络流量 = 在做内存规整，不是在干活）。⚠️ 云助手返回「Agent未运行」只说明 agent 没响应，而控制台收下命令只代表**控制面**收下了 —— 两者都不能用来断定 agent 或系统还活着 |
| 全新浏览器**首次登录**后模型列表和默认模型全空，**刷新一下就全好** | 登录成功后走的是 `router.replace(next)` + `router.refresh()`，**两个都是客户端跳转**。而 `ModelStoreServerSync` / `usePlatformMode` 这类「mount 时拉一次属于我的数据」的客户端组件挂在根 layout（`[locale]/layout.tsx`）上 —— **登录页也在这个 layout 下面**，于是那次拉取发生在还没登录的时候，`/api/platform/providers` 必然 401。客户端跳转不卸载客户端组件，`useEffect(..., [])` 不重跑，于是**整个会话都停留在匿名那次的空结果上**。连带症状：平台托管模式被判成 false，运营 admin 看到了本不该有的 Key 配置表单。之所以长期没人发现：老浏览器的 localStorage 里有 zustand persist 留下的 provider 列表把空结果盖住了，**只有全新 profile（隐身窗口）才暴露** | 登录/注册/退出一律改成**整页跳转**（`window.location.assign`）—— 身份变了，客户端在旧身份下建立的所有缓存就全部作废，这正是整页重载该出现的时刻；修单个组件治不了根，以后任何新组件只要在 mount 时拉一次「属于我的」数据就会重踩。`auth-full-reload.test.ts` 结构性钉死（已反向验证会报红）。顺带：`ModelStoreServerSync` 的裸 `catch {}` 改成带 `console.warn` —— 「请求失败」和「服务端本来就没数据」在界面上长得一模一样（都是空下拉框），而处置完全相反。⚠️ 排查教训：我第一次把它归因成「400ms 的加载闪烁」，被用户用隐身窗口当场证伪；真正定位靠的是**在一次性临时库上原样复现**（`pnpm user:create` 建 owner+运营 admin、把线上 owner 的 `model_store_json` 塞进去、另起一个端口跑 dev），而不是继续读代码猜 |
| owner 在 A 电脑改了模型列表，B 电脑上**永远不更新**，还会把 A 的改动盖回去 | 非托管分支是「本地为空才合并」：`if (providers.length === 0) applyPayload(data)`。B 上本地非空 → 服务端那份永不采用；而防抖写回照常跑 → 把 B 那份旧的写回服务端。owner 恰恰是唯一能改平台列表的人 | `GET /api/user-prefs/model-store` 改返回信封 `{ userId, updatedAt, payload }`，按**服务端时钟**的版本号比较，谁新用谁；本地更新时主动补一次写回 |
| 同一台浏览器换账号，上一个人的 provider 列表出现在下一个人的设置页 | zustand persist 的 key 是固定的 `"model-store"`，**不带 userId**，退出登录也不清它 | 客户端把「这份是谁的 + 对应哪一版」记在独立的 `model-store-sync`（不进 persist），与服务端返回的 `userId` 不符就整体丢弃本地那份。所以接口**没有数据时也要返回 userId** |
| 把 provider 列表全删光，刷新之后**又回来了** | 写回被 `hasPayloadContent()` 挡掉 —— 那个守卫本意是防「拉取还没完成就用空 store 覆盖服务端」，结果连「用户真的删光了」也一并挡住 | 改成只在**拉取失败**时才拒绝写空（`pullSucceeded`）。拉取成功过之后，空就是用户真的删光了，必须存得下去 |
| 默认模型指向一个**已经不存在的模型**，请求照发 | `keepIfProviderExists` 只比 `providerId`：管理员取消勾选某个模型、或换了模型 id 之后，provider 还在、模型没了。下拉框因为 value 匹配不到 option 而显示空，**但 store 里那个 ref 还在**，生成请求带着列表里没有的 modelId 发出去，失败信息只会是上游的「模型不存在」 | 改为 `keepIfModelExists`，判据与 `default-model-picker` 的 `getOptions` 完全一致（模型存在**且** `checked`），取不到就回落到管理员的默认值 |
