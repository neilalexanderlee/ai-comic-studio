...
| dev 控制台常驻一条 `Hydration failed` —— **不要去修业务代码** | 报错指向 base-ui 生成的 `id`（`base-ui-_R_dinekned5rlb_` vs `base-ui-_R_1pinekned5rlb_`），两个都是 React `useId` 的产物，尾部相同、开头不同，即服务端与客户端的树位置不一致。**但它只在 dev server 上出现**：2026-09-05 用同一份代码对比过 —— `/zh`、`/zh/register`、`/zh/settings/prompts` 在本地 dev 必现，在生产构建上**同样这几页零条控制台输出**（并用一条 `console.error` 探针验证过采集本身是通的，不是「抓不到」）。所以它不影响线上，属于 dev 工具链（Next 16 + Turbopack + base-ui 1.2）的产物 | **不动它。** 排查任何 hydration 报错前先做这一步：① 换一个**没改过**的页面看是否同样复现（是 → 与你的改动无关）；② 在**生产构建**上复现一次（不复现 → 只是 dev 噪音）。⚠️ 真正需要自己修的是另一类：在 render 里直接读 `localStorage`/`window` —— 那个会在生产也出问题，见上一条 |
