...
| 把「我连不上服务器」当成「服务器挂了」，据此对生产机做了**强制重启**（等同拔电源）| 本地 VPN / 代理（Clash TUN 之类）会**先在本地接受 TCP 连接**再去拨上游，拨不通就表现为「连上了但对端一个字节都不发」；ICMP 也由 TUN 栈代答。于是「ping 通 + TCP 能建连 + SSH banner 永不返回」这一组现象，**与机器被 OOM 卡死完全同形**。强制重启后现象丝毫未变，才回头做对照实验发现问题在本地 | **决定性判据：连一个安全组根本没放行的端口**（`nc -z <IP> 12345`）。真实网络必然失败；若它也「成功」且耗时 <1ms，说明**本机所有 TCP/ICMP 结果都是伪造的**，不能作为任何判断依据。辅助：`netstat -rn -f inet \| awk '$1=="default"'` 看默认路由是否走 utunN。然后看**阿里云控制台 → 实例 → 监控 → 状态检查**（系统可达性 / 实例状态）—— 那是云侧视角，唯一不受本地网络污染。判断顺序固定为 **① 云侧健康检查 → ② 本地对照实验 → ③ 才怀疑服务器**。要坐实 OOM 只能登进去看 `dmesg -T \| grep -i "out of memory"` |
