...
| 要加闸的 layout 是**客户端组件**时怎么办 | `project/[id]/layout.tsx` 原本是 `"use client"`，读不到 cookie，所以未登录访问项目链接只能得到 404 —— 不泄漏数据，但收藏了链接的人会撞进死胡同 | **拆成「服务端 layout（闸）+ 客户端外壳（UI，内容不动）」**，而不是把守卫下推到每个 page。关键收益：**服务端 layout 的 `params` 里有动态段**（这里是 `[id]`），`next=` 能指回**这个项目**而不是退化成首页。⚠️ 但要指到一个**真实存在的页面**：我第一版指的是 `/project/{id}`，而那一层根本没有 `page.tsx`（只有子路由），用户登录完直接落到 404 —— 比原来的死胡同还糟。改指默认入口 `/episodes`。⚠️ **不要图省事在上一层加 `project/layout.tsx`** —— 那一层拿不到 `[id]`，`next=` 只能退化成首页，等于把「登录后回到原处」这个唯一收益丢掉了。（设置区就是这种情况：`settings/layout.tsx` 拿不到子页 pathname，从 `/settings/prompts` 被踢出去只能回 `/settings`，已知并接受）|
