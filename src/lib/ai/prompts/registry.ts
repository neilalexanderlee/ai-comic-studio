...
四个视角从左到右横向等距排列在纯白画布上，独立分镜互不干扰，统一中景（腰部到头顶），人物全程笔直站立、头部端正、无歪头、无扭头、无倾斜，四个视角比例、大小、构图严格保持一致：
正面 —— 完全 0 度正对观众，标准正面站姿，双臂自然放松垂于两侧，展示完整服装和手持武器
四分之三侧面 ——固定向右精准 45°，严格限定角度，禁止偏大 / 偏小侧面，仅展示面部与身体立体感
侧面轮廓 ——固定向右标准 90° 纯正侧面，角度锁死无偏移，清晰展示鼻子、头发和武器的侧面轮廓
背面 —— 完全 180° 背对镜头，标准后背视角，展示后脑发型、服装背部细节、背部装备`;
...
- 用途：视频生成模型的主要输入——驱动所有动态；必须是自然的散文，适配 Seedance / Kling / Jimeng Video / Veo 等主流AI视频生成模型
...
const FIRST_FRAME_RENDERING_QUALITY = `=== 渲染标准（电影级品质）===
...
背景：【强制必须】渲染完整精细的背景环境——严禁白色/灰色/纯色背景；背景至少占画面30%面积，展现场景纵深、建筑/自然环境细节与大气透视；背景须与场景描述完全吻合
角色：精确匹配参考图外貌与画风；表情生动传达情绪（眉眼嘴角均有具体变化）；姿态富有重量感与力学合理性；服装/皮肤细节准确，褶皱符合受力方向
构图：电影级取景——三分法或黄金比例；前中后层次清晰，有视觉引导线；角色占画面40-70%，背景环境可见，画面有空间纵深感
...
const LAST_FRAME_RENDERING_QUALITY = `=== 渲染标准（电影级品质）===
...
背景：【强制必须】渲染与首帧完全一致的完整背景环境——严禁白色/灰色/纯色背景；同一场景、同一纵深关系、同一大气氛围与光照方向
角色身份与比例：【严格比对首帧和设定图】角色面孔必须是同一人物，体型/身高/肩宽/四肢比例必须与设定图和首帧完全一致，不可随意更改角色比例。服装款式、颜色、每一件配件（腰带、发饰、武器等）均须与设定图完全相符，严禁擅自替换任何服装元素。
情感表现：展示镜头动作结束时的具体情感状态（表情变化）
构图：镜头动作的自然收束，构图完整，为下一个镜头提供自然衔接点；角色占画面40-70%
画质词：masterpiece, best quality, highly detailed, sharp focus, cinematic lighting, 8K resolution
禁止：无文字叠加、无水印、无UI元素、无字幕条、无白色或纯色背景`;
...
const frameGenerateLastDef: PromptDefinition = {
  ...
  buildFullPrompt(sc, params) {
    ...
    lines.push(`第一张附带图像是此镜头的首帧——用于匹配背景环境、布光和画风。`);
    lines.push(`其余附带图像是角色设定图（每张4个视角，名字印在底部）——角色外貌、服装、体型以设定图为最高权威。`);
    lines.push(`若首帧与设定图在角色外貌上有差异（例如首帧为群演场景），以设定图为准。`);
    lines.push("");
...
const SCENE_FRAME_STYLE_MATCHING = `=== 关键：画风匹配 ===
画风由附带的角色设定图决定——精确匹配参考图的视觉风格，不要切换或混合画风。
如无参考图，根据角色描述中指定或暗示的风格推断：
动漫/漫画/anime → 动漫风格；写实/真人/photorealistic → 写实渲染；否则根据角色背景类型推断。`;
...
const sceneFrameGenerateDef: PromptDefinition = {
  ...
  category: "frame",
  slots: [
    slot("style_matching", SCENE_FRAME_STYLE_MATCHING, true),
...
const VIDEO_INTERPOLATION_HEADER = `从首帧平滑插值到尾帧，生成电影级动态画面。

提示词格式遵循 Seedance 官方规范：【主体】+【运动】+【环境】+【运镜】+【美学描述】

运镜描写公式：起幅构图（角色/物体位置与景别）→ 运镜方式与幅度 → 落幅构图（终止位置与景别）
示例："镜头起幅为人物膝盖以下的低角度仰拍，缓缓上摇45°，落幅停于人物上半身中景"

高质量动态描写标准——
• 人物动作：描述关节角度变化、力道传递方向与动作弧线（例："右臂以肩关节为轴外展90°，拳头以弧线轨迹切向斜下方"），而非笼统的"挥拳"
• 微表情：描述具体面部肌肉变化（例："眉心肌肉松弛、眼眸下垂2°、嘴角缓缓向上弧起"），而非"她笑了"
• 动作节奏：分解动作序列（例："接招→化力→反击"），注明速度变化（例："极速冲刺→骤停→爆发性出拳"）
• 运镜技法：优先使用环绕运镜、走入/走出画面、急推/缓拉等电影级切镜；描述运镜起幅与落幅
• 聚焦动态过渡，而非静态外观描述`;
