...
const frameGenerateFirstDef: PromptDefinition = {
  ...
  buildFullPrompt(sc, params) {
    ...
    lines.push(`按以下描述渲染此帧静态画面。⚠️ 物理过滤器：若描述中出现违反重力的构图（如"灯笼链延伸至画面四角"、"向四角辐射"、"延伸至边缘"等），灯笼/旗帜/布料等物体只能向下垂挂或随风水平飘展——请自动修正为符合物理规律的等效构图，保持整体氛围不变：`);
...
const frameGenerateLastDef: PromptDefinition = {
  ...
  buildFullPrompt(sc, params) {
    ...
    lines.push("");
    lines.push(`=== 帧描述（物理规律自动修正）===`);
    lines.push(`按以下描述渲染此帧静态画面。⚠️ 物理过滤器：若描述中出现违反重力的构图（如"灯笼链延伸至画面四角"、"向四角辐射"等），灯笼/旗帜/布料等物体只能向下垂挂或随风水平飘展——请自动修正为符合物理规律的等效构图，保持整体氛围不变：`);
