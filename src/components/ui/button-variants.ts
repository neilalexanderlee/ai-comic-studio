 ...
 * 按钮样式变体。
 *
 * ⚠️ **刻意放在没有 `"use client"` 的独立文件里。**
 * 它原本定义在 `button.tsx` 中，而那个文件顶部有 `"use client"` ——
 * 于是**服务端组件调用 `buttonVariants()` 会在运行时抛错**：
 *   `Attempted to call buttonVariants() from the server but buttonVariants is on the client.`
 * 这是 500，不是编译错误：`tsc` 和 `next build` 都不会报，只有真正渲染到那条分支时才炸。
 *
 * 它本身是纯 cva 配置，没有任何客户端依赖，所以拆出来是无损的。
 * 「把 Link 渲染成按钮样子」是服务端组件的常见需求，需要在两侧都能用。
 */
