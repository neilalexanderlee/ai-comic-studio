...
完整测试入口、浏览器验收步骤和提交完整性检查见 [src/lib/video/README.md](src/lib/video/README.md#剪辑器-harness-维护约定)。
...
- 并行会话改写历史后，先 fetch 两端，再检查本轮提交是否是 HEAD 和两个远端分支的祖先，并核对当前文件与回归结果。祖先检查通过不代表后续没有撤销代码；不要为恢复旧提交而覆盖其他会话的新改动。
