...
```bash
ADMIN_USERNAMES=alice,bob   # 逗号分隔，启动与登录时幂等提升为管理员（只授予、不撤销）
```

**全新的空数据库**且未设该变量时，第一个注册的用户自动成为管理员 —— 自部署装完即可用。
已有数据的库不会触发这条，用 `ADMIN_USERNAMES` 指定即可。

管理员在 `/admin` 有一个后台，可以：

- 生成 / 作废邀请码（可设可用次数与有效期）
- 停用、恢复账号，或授予、取消管理员（停用会立即让该账号的登录态失效）
- 查看最近 24 小时各账号的模型用量
...
开启后，非管理员的设置页不再显示「配置模型 / API Key」，改用管理员配置的模型；
...
这些上限**只作用于使用管理员配置的模型的请求**；自带 Key 的用户不受影响。
