...
- **镜头衔接** — 手动「承接上一镜尾帧 / 上一集尾帧」按钮，把上一镜的真实尾帧直拷为本镜首帧，保证镜间画面连贯
...
**默认是单机匿名可用的** —— clone 下来 `pnpm dev` 就能直接用，不需要注册、不需要配置下面任何一项。
只有当你把服务暴露到公网时，才需要打开这些开关。

### 暴露到公网前必须设置

```bash
REQUIRE_AUTH=1          # 只认签名登录 cookie；不设则任何人加个请求头就能读到别人的数据
APP_BIND=0.0.0.0:3007   # 默认只绑 127.0.0.1（本机可访问）
```

> `AUTH_SECRET` 可以不设 —— 首次需要时会自动生成一把随机密钥并保存在数据目录下的
> `.auth-secret`（Docker 下位于挂载卷内，重启不会把人踢下线）。多实例共享会话时才需要显式指定。

### 谁能注册

```bash
REGISTRATION_MODE=open      # 默认：任何人可自助注册
REGISTRATION_MODE=invite    # 邀请制：注册需要一个邀请码
REGISTRATION_MODE=closed    # 关闭注册
```

不设置时沿用旧开关 `ALLOW_REGISTRATION`（`0` = 关闭，其余 = 开放），行为与升级前完全一致。

> ⚠️ 两者冲突时取更严的：`ALLOW_REGISTRATION=0` 会压过 `REGISTRATION_MODE=invite`。
> 要启用邀请制，请把 `ALLOW_REGISTRATION` 删掉或设为 `1`（启动日志里会有明确提示）。

### 三级权限

| 角色 | 管理后台（邀请码 / 用户 / 用量） | 查看与配置模型 API Key |
|---|---|---|
| `owner` | ✅ | ✅ |
| `admin`（运营） | ✅ | ❌ |
| `user` | ❌ | ❌ |

拆成两级是因为「帮忙拉人、看用量」和「掌握上游 API Key」是两件不该捆绑的事：
你可以让人负责运营而不必把密钥一并交出去。

```bash
ADMIN_USERNAMES=alice,bob   # 逗号分隔，启动与登录时幂等提升为 owner（只授予、不撤销）
```

**全新的空数据库**且未设该变量时，第一个注册的用户自动成为 `owner` —— 自部署装完即可用。
已有数据的库不会触发这条，用 `ADMIN_USERNAMES` 指定即可。
运营 `admin` 由 owner 在管理后台里指派，不走环境变量。

`/admin` 后台可以：

- 生成 / 作废邀请码（可设可用次数与有效期）
- 停用、恢复账号（停用会立即让该账号的登录态失效）
- 查看最近 24 小时各账号的模型用量与估算成本
- **仅 owner**：把成员设为运营管理员或收回
...
```bash
pnpm user:create <用户名> <密码> [owner|admin|user]
```

密码走与注册接口相同的 scrypt 哈希，库里不留明文。已存在的用户名会被拒绝。

### 修改密码

登录后在「设置 → 账号 → 修改密码」。改密后<b>其他设备上的登录会立即失效</b>，
需要用新密码重新登录 —— 怀疑账号泄露时，改一次密码就能把对方踢下线。

### 统一由管理员配置模型（可选）

默认每个用户在设置页填自己的 API Key（BYOK）。若你希望由管理员统一配置、其他人直接使用：

```bash
ALLOW_USER_PROVIDERS=0
```

开启后，只有 `owner` 能看到和配置「模型 / API Key」，其他人（含运营 admin）统一使用
owner 配置好的模型；
可另行设置每人每 24 小时的用量上限与全局并发，避免额度被单个账号占满：

```bash
PLATFORM_DAILY_VIDEO_SECONDS=120   # 视频按秒计（成本按秒产生），0 = 不限制
PLATFORM_DAILY_IMAGE_COUNT=200
PLATFORM_DAILY_MUSIC_COUNT=20
PLATFORM_MAX_INFLIGHT=4            # 全平台同时进行的生成任务数，超出返回 429
```

这些上限**只作用于使用 owner 配置的模型的请求**；自带 Key 的用户不受影响。
...
- **Project** — 项目（剧本、视觉风格、视频比例、最终成片地址）
